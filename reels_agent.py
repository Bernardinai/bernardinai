...
def generate_audio(text, output_filename, title="Bernardinai Reels"):
    # TTS balso generavimas išjungtas - naudojama tik fono muzika
    return False
...
def upload_to_youtube(video_file, video_title, video_description):
    ...
    try:
        ...
        print(
            f"Vaizdo įrašas sėkmingai įkeltas! YouTube ID: {response.get('id')}"
        )
...
def main():
    ...
    if not os.path.exists(FONT_TITLE_FILE):
        print(f"!!! Nerastas šriftas: {FONT_TITLE_FILE}")
        sys.exit(1)
    ...
    for index, entry in enumerate(articles_to_process):
        title = (
            html.unescape(entry.title).replace(". ", ".\u00A0").replace("-", "- ")
        )
        ...
        if has_image:
            ...
            for y in range(height):
                if y > start_fade:
                    opacity = min(
                        245,
                        int(245 * ((y - start_fade) / (height - start_fade))),
                    )
        ...
        if os.path.exists(LOGO_FILE):
            try:
                ...
                draw.rounded_rectangle(
                    bg_box, radius=12, fill=(255, 255, 255, 255)
                )
        ...
        summary_lines = wrap_text(
            summary_text, font_summary, max_text_width, draw
        )
        ...
        draw.rounded_rectangle(
            [center_x - 60, height - 120, center_x + 60, height - 60],
            radius=8,
            fill=(122, 34, 34, 255),
        )
        draw.text(
        ...
        final_clip = CompositeVideoClip(
            [bg_clip, ui_clip], size=(width, height)
        )
        video_clips.append(final_clip)

    youtube_desc += (
        "\nŠiuos straipsnius (o ir kur kas daugiau) raskite atsivertę portalą"
        " Bernardinai.lt!\n"
    )

    final_video = concatenate_videoclips(video_clips, method="compose")

    # Pridedama tik fono muzika (lengvu garsumu, be jokio TTS maišymo)
    if os.path.exists(BG_MUSIC_FILE):
        bg_audio = AudioFileClip(BG_MUSIC_FILE)
        bg_audio = afx.audio_loop(bg_audio, duration=final_video.duration)
        bg_audio = afx.volumex(bg_audio, 0.3)
        final_video = final_video.set_audio(bg_audio)
    else:
        print("Pastaba: bg_music.mp3 nerastas, video bus be garso.")
